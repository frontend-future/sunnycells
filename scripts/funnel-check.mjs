...
await step("q7 stress", "/quiz/diet/weight-loss-difficulty", () => page.getByRole("button", { name: /always stressed/ }).click());
await step("q8 difficulty", "/quiz/diet/height", () => page.getByRole("button", { name: "Yes", exact: true }).click());

await step("q9 height", "/quiz/diet/current-weight", async () => {
  await page.getByLabel("Feet").fill("5");
  await page.getByLabel("Inches").fill("6");
  await page.getByRole("button", { name: /^Continue/ }).click();
});
await step("q10 weight", "/quiz/diet/target-weight", async () => {
  await page.getByLabel("Weight").fill("196");
  await page.getByRole("button", { name: /^Continue/ }).click();
});
await step("q11 target", "/quiz/diet/age", async () => {
  await page.getByLabel("Weight").fill("165");
  await page.getByRole("button", { name: /^Continue/ }).click();
});
await page.screenshot({ path: "shot-q12-age.png" });
await step("q12 age", "/quiz/diet/last-content-weight", async () => {
  await page.getByLabel("Age").fill("47");
  await page.getByRole("button", { name: /^Continue/ }).click();
});
await step("q13 content", "/quiz/diet/daytime-tiredness", () => page.getByRole("button", { name: /1 to 2 years ago/ }).click());
await step("q14 tiredness", "/quiz/diet/thirst-urination", () => page.getByRole("button", { name: /tired all day long/ }).click());
await step("q15 thirst", "/quiz/diet/headaches", () => page.getByRole("button", { name: "No", exact: true }).click());
await step("q16 headaches", "/quiz/diet/moon-face", () => page.getByRole("button", { name: "Yes", exact: true }).click());
await step("q17 moon face", "/quiz/diet/sleep", () => page.getByRole("button", { name: "No", exact: true }).click());
await step("q18 sleep", "/quiz/diet/made-for-you", () => page.getByRole("button", { name: /5 to 6 hours/ }).click());
await step("q19 made for you", "/quiz/diet/email", () => page.getByRole("button", { name: /^Continue/ }).click());
...
await step("q20 email", "/quiz/diet/results/analyzing", () => page.getByRole("button", { name: /Show my results/ }).click());
