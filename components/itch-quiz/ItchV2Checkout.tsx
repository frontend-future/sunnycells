...
import { ITCH_V2_BONUSES, buildItchOrderV2 } from "@/lib/quiz/itchOrderV2";
...
/**
 * The itch v2 checkout: the diet funnel's own shipping-details UX (Information
 * > Payments > Receipt breadcrumbs, a collapsible order summary with bonus line
 * items, sun-yellow accent) cloned onto SC-01 Daily Chews, in place of the
 * plainer EvenCheckout the original itch quiz and the joint quiz still use.
 * Its own file rather than a shared one because the bonuses and the yellow
 * accent are a v2-only test, not something the original itch checkout should
 * inherit. See components/quiz/CheckoutScreen.tsx for the component this
 * mirrors.
 *
 * The plan comes from ITCH_CART_ID (written by ItchPlanCards, shared with the
 * original itch checkout on purpose: one active cart per browser, same
 * convention brainV2Quiz/brainV3Quiz share BRAIN_LADDER_CART_ID). The email
 * comes from itchV2Quiz's own answers instead, since that is where step 18 of
 * this quiz specifically wrote it.
 */
...
const BONUS_ICON: Record<string, IconName> = Object.fromEntries(
  ITCH_V2_BONUSES.map((b) => [b.id, b.icon as IconName]),
);
...
export function ItchV2Checkout({
  ...
  const { answers: quizAnswers } = useAnswers(itchV2Quiz.id);
  const plan = itchPlanById(cart.plan);
  const order = buildItchOrderV2(cart.plan);
  ...
  const notifyAttempt = (shipping: Record<string, string>, stage: "payment" | "purchase") => {
    fetch("/api/notify-purchase", {
      ...
      .catch((err) => console.error("[itch-v2-checkout] notify-purchase failed", err));
  };

  const shippingPayload = () => ({
    email: (f.email ?? quizAnswers.email ?? "").trim(),
  ...
  const submit = () => {
    ...
    /* AddPaymentInfo stays here on purpose: reaching the payment step is precisely
       what that event means. Purchase waits for Submit secure payment. */
