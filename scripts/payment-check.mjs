...
for (const [l, v] of [["First name", "Dana"], ["Last name", "Reyes"], ["Address line 1", "18 Alder Road"], ["Town or city", "Portland"], ["Zip code", "97205"], ["Phone", "+1 503 555 0142"]])
...
for (const [digits, want] of [["4", "Visa"], ["55", "Mastercard"], ["37", "Amex"], ["6011", "Discover"]]) {
  await num.fill(digits);
  await p.waitForTimeout(80);
  const shown = await p.evaluate(() => {
    const i = [...document.querySelectorAll("input")].find((x) => x.autocomplete === "cc-number");
    return i.parentElement.querySelector("span")?.textContent ?? "";
  });
