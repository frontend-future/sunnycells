...
import { JOINT_CART_ID, JOINT_PLANS, jointPlanBullets } from "@/lib/quiz/jointLadder";
...
/**
 * The itch quiz's PlanCards, styled and wired identically, selling SC-02 Hip
 * & Joint Chews on the same paid 1/3/6 month ladder instead of a quantity of
 * pouches. Kept as its own component rather than a generalised PlanCards,
 * same reasoning ItchPlanCards documents.
 */
export function JointPlanCards({
  destinationHref = "/quiz/joint/results/checkout",
  ctaLabel = "Try now",
  plans = JOINT_PLANS,
  quizId = jointQuiz.id,
}: {
  ...
  const choose = (p: Plan) => {
    ...
    chosen.current = true;
    writeAnswer(JOINT_CART_ID, "plan", p.id);
    writeAnswer(JOINT_CART_ID, "planPrice", String(p.price));
    writeAnswer(JOINT_CART_ID, "planMonths", String(p.months));
    trackMetaEvent(
      "InitiateCheckout",
      {
        currency: "USD",
