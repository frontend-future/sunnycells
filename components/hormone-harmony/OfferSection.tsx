  ...
  return (
    <label className={`${styles.planChoice} ${featured ? styles.planFeatured : ""} ${selected ? styles.planSelected : ""}`}>
      ...
      <span className={styles.planText}>
        ...
        <span>{plan.sub}</span>
        <small>Save ${savings} per delivery</small>
      </span>
  ...
  return (
    <section id="plans" className={`${styles.section} ${styles.offer}`} aria-labelledby="offer-title">
      <div className={styles.offerVisual}>
        ...
        <h2 id="offer-title">Make room for one morning habit.</h2>
        <Image
          key={selected.id}
          ...
          priority={false}
        />
        <p>One pouch contains 30 servings. Every plan includes free shipping.</p>
      </div>
