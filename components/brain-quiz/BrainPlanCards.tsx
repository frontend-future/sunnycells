"use client";

import { useRouter } from "next/navigation";
import { useRef, useState } from "react";
import Image from "next/image";
import { Button } from "@/components/core/Button";
import { Badge } from "@/components/core/Badge";
import { Icon } from "@/components/core/Icon";
import { PRODUCT } from "@/lib/products/brain-memory";
import { brainQuiz } from "@/lib/quiz/brain";
import { BRAIN_LADDER_CART_ID, BRAIN_PLANS, brainPlanBullets } from "@/lib/quiz/brainLadder";
import type { Plan } from "@/lib/quiz/plans";
import { readAnswers, writeAnswer } from "@/lib/quiz/store";
import { trackMetaEvent } from "@/lib/meta";

function PlanCard({
  p, big = false, hover, onHover, ctaLabel, onChoose,
}: {
  p: Plan;
  big?: boolean;
  hover: string;
  onHover: (id: string) => void;
  ctaLabel: string;
  onChoose: (p: Plan) => void;
}) {
  const on = p.best || hover === p.id;
  return (
    <div
      onMouseEnter={() => onHover(p.id)}
      onMouseLeave={() => onHover("")}
      style={{
        position: "relative",
        display: "flex",
        flexDirection: "column",
        gap: "var(--space-4)",
        padding: "var(--space-6) var(--space-5) var(--space-5)",
        background: on ? "var(--sky-tint)" : "var(--white)",
        border: `2px solid ${on ? "var(--ink)" : "var(--border-hairline)"}`,
        borderRadius: "var(--radius-card)",
        transition: "background var(--duration-fast) var(--ease-standard)",
      }}
    >
      {/* Skipped on the big featured card: its own banner above already says
          "recommended", so a second floating flag repeats the same claim. */}
      {p.flag && !big ? (
        <div style={{ position: "absolute", top: -16, left: "50%", transform: "translateX(-50%)" }}>
          <Badge tone={p.best ? "ink" : "sun"}>{p.flag}</Badge>
        </div>
      ) : null}

      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: big ? "var(--size-body-lg)" : "var(--size-body)", fontWeight: 800 }}>{p.label}</div>
        <div style={{ marginTop: 2, fontSize: "var(--size-meta)", color: "var(--ink-60)" }}>{p.sub}</div>
      </div>

      {/* The bottle count matches what actually arrives: a bundle shot for
          3 and 6, one bottle for the monthly plan. */}
      <Image
        src={p.image}
        alt={`${p.months} ${p.months === 1 ? "bottle" : "bottles"} of ${PRODUCT.name}`}
        width={1200}
        height={900}
        style={{ width: "100%", height: "auto", maxHeight: big ? 210 : 150, objectFit: "contain" }}
      />

      {/* Price and the struck list price on one line, so the comparison reads
          in a single glance rather than over two rows. */}
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: "var(--space-3)", flexWrap: "wrap" }}>
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 900, fontSize: big ? 52 : 40, letterSpacing: "var(--tracking-display)", lineHeight: 1 }}>
          ${p.price}
          {p.months > 1 && (
            <span style={{ fontFamily: "var(--font-text)", fontSize: "var(--size-meta)", fontWeight: 600, color: "var(--ink-60)", letterSpacing: 0 }}>
              /bottle
            </span>
          )}
        </span>
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: big ? 30 : 26, color: "var(--ink-60)", textDecoration: "line-through", letterSpacing: "-0.02em" }}>
          ${p.compareAt * p.months}
        </span>
      </div>
      {/* The standing first-order term, stated on the card rather than left to
          be inferred from the struck price. */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "var(--space-2)", marginTop: -6 }}>
        <span
          aria-hidden="true"
          style={{
            flex: "none", width: 20, height: 20, borderRadius: "50%",
            background: "var(--status-success)", color: "var(--white)",
            display: "flex", alignItems: "center", justifyContent: "center",
          }}
        >
          <Icon name="check" size={13} strokeWidth={3.5} />
        </span>
        <span style={{ fontSize: "var(--size-meta)", fontWeight: 800, color: "var(--status-success)" }}>
          50% off auto-applied today
        </span>
      </div>

      <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 4 }}>
        {brainPlanBullets(p).map((b) => (
          <li key={b} style={{ fontSize: "var(--size-meta)", color: "var(--ink-80)", textAlign: "center" }}>
            {b}
          </li>
        ))}
      </ul>

      <div style={{ marginTop: "auto", paddingTop: "var(--space-3)" }}>
        <Button fullWidth variant={p.best ? "primary" : "outline"} onClick={() => onChoose(p)}>
          {ctaLabel}
        </Button>
        <div style={{ marginTop: "var(--space-3)", textAlign: "center", fontSize: "var(--size-meta)", color: "var(--ink-60)" }}>
          Cancel anytime. Free shipping.
        </div>
      </div>
    </div>
  );
}

/**
 * The diet funnel's PlanCards, styled and wired identically, selling Brain &
 * Memory Power Boost on the same paid 1/3/6 month ladder instead of a quantity
 * of pouches. Kept as its own component rather than a generalised PlanCards,
 * since PlanCards hardcodes the diet quiz's id, cart and "pouch" wording rather
 * than taking them as props.
 */
export function BrainPlanCards({
  destinationHref = "/quiz/brain/v2/results/checkout",
  ctaLabel = "Try now",
  plans = BRAIN_PLANS,
  /* Whichever quiz's email capture actually fed this page: the original quiz, its
     v2 clone, or the brain age quiz all keep their own answers, so the one to read
     back here has to be told rather than assumed. */
  quizId = brainQuiz.id,
  /* "featured" leads with the best plan alone, banner and all, with the other
     lengths tucked behind a "Compare other plan options" toggle, the way a single
     strong recommendation reads better than three equal boxes. Defaults to the
     original even grid so v2 keeps its exact current layout. */
  layout = "grid",
}: {
  destinationHref?: string;
  ctaLabel?: string;
  plans?: Plan[];
  quizId?: string;
  layout?: "grid" | "featured";
}) {
  const router = useRouter();
  const [hover, setHover] = useState("");
  const [comparing, setComparing] = useState(false);

  /* One click, one event, same guard PlanCards uses: nothing unmounts the card
     between the tap and the route change, so a double tap would otherwise fire
     InitiateCheckout twice with two event ids, which Meta cannot dedupe. */
  const chosen = useRef(false);

  const choose = (p: Plan) => {
    if (chosen.current) return;
    chosen.current = true;
    writeAnswer(BRAIN_LADDER_CART_ID, "plan", p.id);
    writeAnswer(BRAIN_LADDER_CART_ID, "planPrice", String(p.price));
    writeAnswer(BRAIN_LADDER_CART_ID, "planMonths", String(p.months));
    trackMetaEvent(
      "InitiateCheckout",
      {
        currency: "USD",
        value: p.price * p.months,
        content_ids: [p.id],
        content_type: "product",
        content_name: `${PRODUCT.name} ${p.label}`,
      },
      /* The quiz captured an email several steps back. Passing it here is what
         lets Meta match this event to a person rather than a cookie. */
      { email: readAnswers(quizId).email },
    );
    router.push(destinationHref);
  };

  if (layout === "featured") {
    const best = plans.find((p) => p.best) ?? plans[0];
    const rest = plans.filter((p) => p !== best);

    return (
      <div style={{ maxWidth: 420, margin: "0 auto" }}>
        <div
          style={{
            textAlign: "center",
            background: "var(--zest)",
            color: "var(--white)",
            fontSize: "var(--size-meta)",
            fontWeight: 700,
            letterSpacing: "var(--tracking-caps)",
            textTransform: "uppercase",
            padding: "var(--space-3)",
            borderRadius: "var(--radius-card) var(--radius-card) 0 0",
          }}
        >
          Your recommended plan
        </div>
        <div style={{ border: "2px solid var(--zest)", borderTop: "none", borderRadius: "0 0 var(--radius-card) var(--radius-card)", padding: 2 }}>
          <PlanCard p={best} big hover={hover} onHover={setHover} ctaLabel={ctaLabel} onChoose={choose} />
        </div>

        {rest.length > 0 ? (
          <>
            <button
              type="button"
              onClick={() => setComparing((c) => !c)}
              style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                gap: "var(--space-2)",
                width: "100%",
                margin: "var(--space-6) 0 0",
                padding: "var(--space-3)",
                background: "transparent",
                border: "none",
                fontSize: "var(--size-body)",
                fontWeight: 700,
                textDecoration: "underline",
                cursor: "pointer",
              }}
            >
              Compare other plan options
              <Icon
                name="chevron-down"
                size={18}
                style={{ transform: comparing ? "rotate(180deg)" : undefined, transition: "transform var(--duration-fast) var(--ease-standard)" }}
              />
            </button>

            {comparing ? (
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))",
                  gap: "var(--space-5)",
                  marginTop: "var(--space-6)",
                }}
              >
                {rest.map((p) => (
                  <PlanCard key={p.id} p={p} hover={hover} onHover={setHover} ctaLabel={ctaLabel} onChoose={choose} />
                ))}
              </div>
            ) : null}
          </>
        ) : null}
      </div>
    );
  }

  return (
    <div
      style={{
        display: "grid",
        gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))",
        gap: "var(--space-5)",
        alignItems: "stretch",
      }}
    >
      {plans.map((p) => (
        <PlanCard key={p.id} p={p} hover={hover} onHover={setHover} ctaLabel={ctaLabel} onChoose={choose} />
      ))}
    </div>
  );
}
