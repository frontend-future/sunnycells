...
> **This is the source brief, kept verbatim.** In this repo the tokens live in
> `app/tokens/` (wired up in `app/globals.css`), the components are TypeScript in
> `components/`, and `app/page.tsx` is the live specimen of everything below. The
> `guidelines/` and `ui_kits/` folders referenced in the Index at the bottom were
> preview-only HTML and were not ported.
