...
} from "@/lib/products/anytime-calm";
import { calmQuiz } from "@/lib/quiz/calm";
import { readAnswers, writeAnswer } from "@/lib/quiz/store";

/** The diet funnel's PlanCards, selling the tub on a 1, 3 and 6 month ladder. */
export function CalmPlanCards({
  destinationHref,
  ctaLabel = "Try now",
  ...
  const choose = (p: SupplyPlan) => {
    ...
    trackMetaEvent(
      ...
      { email: readAnswers(calmQuiz.id).email },
    );
