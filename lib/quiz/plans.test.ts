...
test("bullets count servings and pouches from the plan itself", () => {
  ...
  assert.deepEqual(one, ["30 servings", "Less than $1 / day", "1 pouch delivered", "Delivered fresh monthly"]);
  assert.deepEqual(three, ["90 servings", "Less than $1 / day", "3 pouches delivered", "Delivered fresh every 60 days"]);
  assert.deepEqual(six, ["180 servings", "Less than $1 / day", "6 pouches delivered", "Delivered fresh every 90 days"]);
});
